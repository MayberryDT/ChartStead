...
- [ ] The application remains demonstrable when Airtable is disabled or unavailable.
- [ ] Desktop and mobile checks cover all public and speaker surfaces; organizer workflows remain usable at their supported widths.
- [ ] Keyboard and automated accessibility checks cover required forms, queues, dialogs, task flows, schedule movement, and public pages.
- [ ] Performance checks with realistic seeded volume show responsive submission-list, review, and schedule interactions.
- [ ] Loading, empty, error, retry, and partial states preserve context and provide clear next actions.
- [ ] Security checks cover role boundaries, signed-link expiry/revocation, upload authorization, public-data filtering, and demo bypass isolation.
- [ ] Deployment, environment variables, migrations, seed/reset, provider setup, verification, and walkthrough commands are documented without secrets.
- [ ] API documentation covers authenticated vertical-slice operations and representative examples.
- [ ] If time remains after all checks pass, implement the smallest high-value remainder: review coverage sorts, draft handling, communication templates, resources, or table power.
...
- 2026-08-12 — frontier-reconcile: Still blocked on: Course Check 10 (ready-for-agent).
