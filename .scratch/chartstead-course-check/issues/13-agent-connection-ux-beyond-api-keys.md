...
- [ ] Preserve Course Check 08 semantics: distinct agent principals, per-event stage scopes, propose-only by default, delegated vs autonomous modes, live revocation, initiating-human provenance, frozen plans, no privileged bypass.
- [ ] Actor labels always read as agent-on-behalf-of-human in organizer UI (never bare agent name alone when provenance exists).
- [ ] Connection path covers **full organizer API parity** (ordinary writes + Course Check), not Course Check alone — same capability surface as an authorized admin when scopes allow.
- [ ] Settings (or a dedicated Agents surface) guides connect → grant stages/mode → verify → revoke in plain language; no bare “API key” as the only happy path.
- [ ] Optional: one-click “test connection” that runs a read-only Course Check list and shows success/failure in the UI.
- [ ] Document how to connect at least two real agent hosts (e.g. Claude Code / Claude desktop MCP, Cursor, or ChatGPT) with screenshots-level steps.
- [ ] Long-lived bearer keys remain available as an advanced/fallback control for CI and custom automation, clearly labeled secondary.
- [ ] Competition-build boundary respected: MCP was out of scope for the competition spine; this ticket is the deliberate post-08 productization of agent connection UX.
- [ ] Contract tests cover connect, scope grant, revoke, and at least one agent-host-shaped session (MCP tool or OAuth token exchange) producing the same Course Check outcomes as the v1 HTTP path.
