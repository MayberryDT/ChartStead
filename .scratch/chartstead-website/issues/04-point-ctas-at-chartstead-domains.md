...
- [ ] `site.url`, canonical tags, sitemap, and social URLs use `https://chartstead.com` (repo config is stale; live site is already on that domain).
- [ ] `site.demoUrl`, `nav.primaryCta`, and the homepage close CTA use `https://demo.chartstead.com/demo`.
- [ ] The second CTA uses the Website 03 example-event URL on a ChartStead domain.
- [ ] No shipped marketing page still links a `mayberrydt.workers.dev` demo or site URL as the visitor destination.
- [ ] External-link attributes, notes, and Open Graph URLs match the first-party domains.
- [ ] A Tailscale or production preview plus a CTA link checklist are recorded before `in-review`.
