...
test("evaluator entry switches among organizer, scoped reviewer, and signed-link speaker", async ({
  ...
  await expect(page).toHaveURL(new RegExp(`/e/${eventId}/submissions\\?track=agents`));
  ...
  expect((await reviewer.json()).principal).toMatchObject({
    ...
    role: "reviewer",
    trackIdsByEvent: { [eventId]: ["agents"] },
  });
  ...
  const detailPath = `/api/events/${eventId}/organizer/proposals/SUB-AEWF0001`;
  const detail = await (await page.request.get(detailPath)).json();
  const saved = await page.request.patch(`${detailPath}/review`, {
    data: {
      status: "maybe",
      committeeNote: "Keep in the agents-track comparison set.",
  ...
  await expect(page.getByRole("heading", { name: "AI Engineer World's Fair 2026" })).toBeVisible();
  await expect(page.getByText(/Welcome, Nora Ellison/)).toBeVisible();
  await expect(
    page.getByLabel("Portal summary").getByText("Shipping reliable agent workflows in production"),
  ).toBeVisible();
